...
(async () => {
        ...
        window.MassSim = {
            async simulateOne(className) {
                ...
                return {
                    ...
                    floor: currentFloor,
                    gold: player.gold,
                    causeOfDeath: gameState === 'VICTORY' ? 'VICTORY' : (document.getElementById('go-killer')?.innerText.split('killed by ')[1] || (gameState === 'PLAYER_DEAD' ? "Unknown" : (turns >= maxTurns ? "Timeout" : "Ongoing"))),
                    turns,
    ...
    for (let i = 0; i < totalRuns; i++) {
        ...
        results.push(res);
        if ((i+1)%25 === 0) console.log(`📉 Progress: ${i+1}/${totalRuns}...`);
    }
